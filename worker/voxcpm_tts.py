...
_VOICE_REFERENCES = {
    "fengge": _PROJECT_ROOT / "assets" / "voice_samples" / "fengge_clean.wav",
}

_VOICE_TRANSCRIPTS = {
    "fengge": _PROJECT_ROOT / "assets" / "voice_samples" / "fengge_clean.txt",
}

_VOICE_DESIGNS = {
    # Fan-made voice design: similar stage temperament, not an exact clone of a real person.
    "kunkun": (
        "原创二十多岁中文男声，音高偏中高，音色清亮柔和，有自然少年感；胸腔共鸣较轻，"
        "轻微气声，吐字清楚但没有播音腔；语速中等偏慢，停顿从容，尾音自然收住，"
        "情绪真诚内敛，偶尔带一点轻微笑意和舞台感；不沙哑，不低沉，不厚重，"
        "不模仿任何真实人物的独特声纹"
    ),
}
...
class VoxCPMHttpTTS(TTS):
    def __init__(
        ...
        *,
        ...
        self._reference_audio_base64: str | None = None
        self._prompt_text: str | None = None

        reference = _VOICE_REFERENCES.get(voice)
        if reference is not None:
            if not reference.exists():
                raise FileNotFoundError(f"VoxCPM reference audio not found: {reference}")
            self._reference_audio_base64 = base64.b64encode(reference.read_bytes()).decode("ascii")
            transcript = _VOICE_TRANSCRIPTS.get(voice)
            if transcript is not None:
                if not transcript.exists():
                    raise FileNotFoundError(f"VoxCPM reference transcript not found: {transcript}")
                self._prompt_text = transcript.read_text(encoding="utf-8").strip()

        if not self._opts.style:
            self._opts.style = _VOICE_DESIGNS.get(voice, "")
    ...
    async def _get_server_sample_rate(self) -> int:
        if self._server_sample_rate is not None:
            return self._server_sample_rate

        sess = await self._ensure_session()
        async with sess.get(f"{self._opts.url}/info") as resp:
            if resp.status >= 400:
                body = await resp.text()
                raise RuntimeError(f"VoxCPM /info failed ({resp.status}): {body[:200]}")
            data = await resp.json()
        self._server_sample_rate = int(data["model"]["sample_rate"])
        return self._server_sample_rate
...
class _VoxCPMSynthesizeStream(SynthesizeStream):
    ...
    async def _run(self, output_emitter) -> None:
        ...
        async def send_sentence(text: str) -> None:
            ...
            if self._vox._reference_audio_base64 is not None:
                payload.update(
                    {
                        "ref_audio_wav_base64": self._vox._reference_audio_base64,
                        "ref_audio_wav_format": "wav",
                    }
                )
                if self._vox._prompt_text:
                    payload.update(
                        {
                            "prompt_wav_base64": self._vox._reference_audio_base64,
                            "prompt_wav_format": "wav",
                            "prompt_text": self._vox._prompt_text,
                        }
                    )
            ...
            for attempt in range(3):
                try:
                    ...
                    async with sess.post(
                        f"{self._vox._opts.url}/generate",
                        json=payload,
                    ) as resp:
                        if resp.status >= 400:
                            body = await resp.text()
