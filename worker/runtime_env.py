...
# 默认直连。只有显式配置 EGRESS_PROXY_URL 时才启用代理，避免本机没有
# 1087 代理服务时把 DeepSeek/MiniMax 等可直连 API 全部送进失效端口。
_DEFAULT_EGRESS_PROXY = os.getenv("EGRESS_PROXY_URL") or None
...
def configure_egress_proxy(proxy_url: str | None = _DEFAULT_EGRESS_PROXY) -> None:
    """按需把显式配置的代理写入 http_proxy/https_proxy env。

    必须在 import google.genai / httpx / aiohttp 之前调用。
    未配置 EGRESS_PROXY_URL 时保持直连。
    """
    if not proxy_url:
        return
